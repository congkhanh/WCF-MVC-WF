...
using MVCKLShop.Models;
using System.Net.Mail;
using System.Net;
using MVCKLShop.ServiceReferenceSanPham;
using MVCKLShop.ServiceReferenceDatHang;
using MVCKLShop.ServiceReferenceUser;
using MVCKLShop.ServiceReferenceCTDonHangCustom;


namespace MVCKLShop.Controllers
{
    public class GioHangController : Controller
    {
        ...
        public ActionResult Index()
        {
            try
            {
                int num = MVCKLShop.Models.MySession.GioHang.Count;
                if (num > 0)
                {
                    return View();
                }
        ...
        {
            //Lưu các mã sản phẩm
            string ma = Session[MVCKLShop.Models.MySession.MaSanPham] + id.ToString();
            Session[MVCKLShop.Models.MySession.MaSanPham] = ma;

            //Số lượng sản phẩm có trong giỏ hàng
            int sl = 1 + Convert.ToInt32(Session[MVCKLShop.Models.MySession.TongSL]);
            Session[MVCKLShop.Models.MySession.TongSL] = sl.ToString();
        ...
        public ActionResult UpdateCart(FormCollection formCollection)
        {
            ...
            Session[MVCKLShop.Models.MySession.TongSL] = "0";
            ...
            for (int i = 0; i < MySession.GioHang.Count; i++)
            {
                if (MySession.GioHang[i].MaSanPham == masp)
                {
                    if (sl < 1)
                    {
                        ...
                        Session[MVCKLShop.Models.MySession.TongSL] = ((Convert.ToInt32(Session[MVCKLShop.Models.MySession.TongSL]) + 1)).ToString();
                        ...
                        if (sl < Sol)
                        {
                            ...
                            Session[MVCKLShop.Models.MySession.TongSL] = ((Convert.ToInt32(Session[MVCKLShop.Models.MySession.TongSL]) + sl)).ToString();
                ...
                {
                    Session[MVCKLShop.Models.MySession.TongSL] = ((Convert.ToInt32(Session[MVCKLShop.Models.MySession.TongSL]) + MySession.GioHang[i].SoLuong)).ToString();
        ...
        public ActionResult SendMailCart()
        {
            ...
            MVCKLShop.Models.MySession.TongTien = 0;
